...
export function getCircuitState(provider: {
  ...
  const threshold = provider.circuit_breaker_threshold ??
    CIRCUIT_BREAKER_THRESHOLD_DEFAULT;
  if (provider.consecutive_failures < threshold) return "closed";

  const cooldownMinutes = provider.circuit_breaker_cooldown_minutes ??
    CIRCUIT_BREAKER_COOLDOWN_DEFAULT;
  ...
  switch (condition.operator) {
    ...
    case "in":
      if (Array.isArray(condValue)) {
        return condValue.map(String).includes(String(fieldValue));
      }
      return false;
    case "contains":
    ...
    case "regex":
      try {
        return new RegExp(String(condValue)).test(String(fieldValue));
      } catch {
        return false;
      }
    default:
...
function evaluateRouteConditions(
  route: RouteWithProfile,
  context: RouteContext,
): boolean {
  if (!route.route_conditions || route.route_conditions.length === 0) {
    return true;
  }
...
export function selectRoute(
  ...
  if (context?.data_policy) {
    ...
    const policyFiltered = eligible.filter((r) => {
      if (!r.route_data_policies || r.route_data_policies.length === 0) {
        return false;
      }
      return r.route_data_policies.some((rdp) =>
        rdp.data_policies?.name === policyName &&
        (rdp.data_policies.allowed_regions.length === 0 || !context.region ||
          rdp.data_policies.allowed_regions.includes(context.region))
      );
    });
  ...
  if (context) {
    ...
    if (eligible.length === 0) {
      eligible = routes.filter((r) =>
        !r.route_conditions || r.route_conditions.length === 0
      );
    }
    if (eligible.length === 0) {
      throw new Error(
        "No active routes available (all conditions filtered out)",
      );
    }
  }
  ...
  const isRouteHealthy = (r: RouteWithProfile): boolean => {
    ...
    if (isCircuitOpen(provider)) {
      circuitBreakerSkipped.push(
        `${provider.type}/${r.model_profiles.provider_model_name}`,
      );
      return false;
    }
    return true;
  ...
  const primaryRoutes = eligible.filter((r) =>
    r.strategy === "primary" && isRouteHealthy(r)
  );
  const canaryRoutes = eligible.filter((r) =>
    r.strategy === "canary" && isRouteHealthy(r)
  );

  if (canaryRoutes.length > 0 && primaryRoutes.length > 0) {
    const canaryTotalWeight = canaryRoutes.reduce(
      ...
      for (const route of canaryRoutes) {
        ...
        if (random < cumulative) {
          return {
            ...
            dataPolicyFilteredCount,
          };
        }
      }
      return {
        selectedRoute: canaryRoutes[0],
        isCanary: true,
        circuitBreakerSkipped,
        conditionFilteredCount,
        dataPolicyFilteredCount,
  ...
  if (fallbackRoutes.length > 0) {
    return {
      selectedRoute: fallbackRoutes[0],
      isCanary: false,
      circuitBreakerSkipped,
      conditionFilteredCount,
      dataPolicyFilteredCount,
